<template>
    <div class="chats-block">
        ...
        <div class="top-menu-horizontal-block">
            <TopMenuHorizantal :menuItems="menuItems" v-if="!searchState" :class="{ search_deactivate: !searchState }">
            </TopMenuHorizantal>
            <TopMenuHorizantal :menuItems="menuSearchItems" v-if="searchState" :class="{ search_activate: searchState }">
            </TopMenuHorizantal>
        </div>

        <div class="chat-tab-folder-items-block">

            <div class="chat-folder-chat-items" v-if="!searchState" :class="{ search_deactivate: !searchState }">
                <div class="tab-chat-items tab-folder-items scrollbar-y" v-for="test in tests">
                    <h1 style="text-align:center;"> test {{ test.name }}</h1>
                    <UserPeer v-for="user in [1, 2, 3, 4, 5, 6, 7, 8, 9, 5, 7, 8, 5, 6, 3, 6, 6]"></UserPeer>
                </div>
            </div>

            <div class="chat-folders-search-items" v-if="searchState" :class="{ search_activate: searchState }">
...
.chat-folder-chat-items,
.chat-folders-search-items {
    display: flex;
    height: 100%;
    overflow: hidden;
}
